...
CommonHotkeys.setExitFunc(onExit)
...
MButton:: {
	...
	t := Toast("Ready to send RGB color to a color dialog: " rgbColor "`nMiddle-click again to send").show()
	KeyWait("MButton", "D")
...
buildGui() {
	global GuiId, GuiWidth, GuiHeight, LabelHex, LabelRGB
	; Create gui and set overall properties
	guiObj := Gui("-Caption +Border +AlwaysOnTop +ToolWindow")
	GuiId := guiObj.Hwnd ; GuiId := window handle (which can be used with ahk_id)
	guiObj.SetFont("s" FONT_SIZE, FONT_NAME)
	guiObj.MarginX := GUI_SPACING
	guiObj.MarginY := GUI_SPACING
	...
	LabelHex := guiObj.Add("Text", "Center", defaultText)
	LabelRGB := guiObj.Add("Text", "Center y+" GUI_SPACING " R3", defaultText) ; RGB table goes a little lower and has 3 rows

	; Auto-size gui and store off size in globals
	guiObj.Show("AutoSize Hide") ; Hide - don't actually show it until we have real values in place.
...
getRGBText(hexColor) {
	...
	tt := TextTable().setColumnDivider(" ").setColumnAlignment(2, TextAlignment.Right)
	tt.addRow("R", RGB["R"])
	tt.addRow("G", RGB["G"])
	tt.addRow("B", RGB["B"])
...
invertColor(hexColor) {
	...
	return StrUpper(finalR finalG finalB)
}
...
moveGui(mouseX, mouseY) {
	...
	guiObj := GuiFromHwnd(GuiId)
	guiObj.Show("NoActivate x" guiX " y" guiY)
}

finishGui() {
	; Destroy the gui since we're done picking the color.
	guiObj := GuiFromHwnd(GuiId)
	guiObj.Destroy()
