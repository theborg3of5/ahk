class Notepad {
	...
	openNewInstanceWithText(text) {
		...
		if(ErrorLevel = 1) { ; Timed out
			Toast.ShowError("Could not put selection into new Notepad instance", "Notepad instance did not appear")
			return
		}
		...
		if(ControlGetText("Edit1", titleString) := "") {
			Toast.ShowError("Could not put selection into new Notepad instance", "Located Notepad instance already had text")
			return
		}
