...
; Format the given text as code.
formatSelectionAsCode() {
	; windowName := getWindowSetting("NAME") ; Active window's matching name in window settings (windows.tl).
	; if(!doesWindowSupportFormatAsCode(windowName)) ; GDB TODO remove if not needed
		; return
	
	windowName := getWindowSetting("NAME") ; Active window's matching name in window settings (windows.tl).
	
	if(windowName = "EMC2 DLG" || windowName = "EMC2 XDS") {
		Send, ^e
		
	} else if(windowName = "Outlook") {
		Send, ^+s
	...
	} else if(windowName = "Word") {
		; GDB TODO
		
	} else if(windowName = "OneNote") {
		oneNoteCustomStyles()
		Send, {Enter}
		
	}
}
; doesWindowSupportFormatAsCode(name) { ; GDB TODO remove if not needed.
	; windowNamesAry := []
	; windowNamesAry["OneNote"]  := ""
	; windowNamesAry["Outlook"]  := ""
	; ; windowNamesAry["Word"]     := ""
