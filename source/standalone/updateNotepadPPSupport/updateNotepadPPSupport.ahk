...
updateCompletionXML_TL(templatePath, tlMembers) {
	...
	; Make everything just `n line endings, so it matches the values generated by keywords. This way
	; everything becomes `r`n when we save it to the file - if we don't do this, we don't convert
	; line endings at all (presumably because some already have `r`n).
	templateXML := FileRead(templatePath).replace("`r`n", "`n")
